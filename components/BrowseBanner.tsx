...
const BrowseBanner = () => {
  ...
  const searchTags = (tag: number) => {
    const params = new URLSearchParams(searchParams.toString());

    if (tag === Number(initialGenre)) {
      params.delete("genre");
      router.push(`/browse?${params.toString()}`, { scroll: false });
    } else {
      params.set("genre", String(tag));
      router.push(`/browse?${params.toString()}`, { scroll: false });
    }
  };
          ...
          <div className="w-full max-w-3xl min-[375px]:mt-10 mt-8">
            <form className="relative group" onSubmit={handleSearch}>
              ...
              <input
                ...
                className={`w-full sm:text-base min-[375px]:text-sm text-[13px] py-3 sm:pl-14 min-[375px]:pl-11 pl-10 sm:pr-24 ${search ? "min-[375px]:pr-[85px] pr-20" : "pr-12"} bg-white/5 border border-white/10 rounded-xl text-white placeholder:text-white/40 font-medium focus:outline-none focus:border-[#e91e3f]/50 focus:ring-2 focus:ring-[#e91e3f]/20 transition-all duration-300 group-hover:border-white/20`}
              />
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 mt-4">
              ...
              {popularGenres.map((tag) => (
                <button
                  key={tag.id}
                  onClick={() => searchTags(tag.id)}
                  className={`px-3 py-1 min-[375px]:text-xs text-[10px] border ${Number(initialGenre) === tag.id ? "font-medium border-[#e91e3f] text-[#e91e3f]" : "text-white/60 hover:text-white border-white/10"} bg-white/5 hover:bg-white/10 rounded-full transition-all`}
                >
                  {tag.name}
                </button>
              ))}
